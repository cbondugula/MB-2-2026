...
import fs from 'fs';
import { db } from './db';
import { langExtractExtractions, langExtractTemplates } from '../shared/schema';
...
export interface HealthcareExtraction {
  id: string;
  type: 'medication' | 'diagnosis' | 'symptom' | 'lab_result' | 'procedure' | 'dosage';
  text: string;
  startOffset: number;
  endOffset: number;
  confidence: number;
...
export class LangExtractService {
  ...
  constructor() {
    ...
    this.scriptPath = path.join(process.cwd(), 'python_scripts', 'langextract_processor.py');
...
`;
    ...
    console.log('✅ Python LangExtract script created');
  }

  async extractText(request: LangExtractRequest): Promise<LangExtractResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (request.instructions) {
        args.push('--instructions', request.instructions);
      }

      if (request.examples) {
        args.push('--examples', JSON.stringify(request.examples));
      }
      ...
      pythonProcess.on('close', (code) => {
        ...
        try {
          ...
          resolve({
            ...
            visualizationHtml: result.visualization_html || '',
            confidence: result.confidence || 0,
            modelUsed: result.model_used || 'gemini-2.5-flash',
            processingTime: processingTime
          });
        } catch (error) {
          reject(new Error(`Failed to parse Python response: ${(error as Error).message}`));
        }
      });
  ...
  async saveExtraction(
    ...
    const [extraction] = await db.insert(langExtractExtractions).values({
      userId,
      projectId,
      name,
      description,
      ...
      results: result.extractions,
      confidence: result.confidence,
      modelUsed: result.modelUsed,
      visualizationHtml: result.visualizationHtml,
      tags: [request.extractionType, 'healthcare'],
      status: 'completed'
    }).returning();
  ...
  async getExtractions(userId: string, projectId?: number) {
    if (projectId) {
      return await db
        .select()
        .from(langExtractExtractions)
        .where(eq(langExtractExtractions.userId, userId))
        .where(eq(langExtractExtractions.projectId, projectId));
    }
  ...
  async getTemplates() {
    return await db.select().from(langExtractTemplates).execute();
  }

  async getAvailableTemplates(): Promise<any> {
    return new Promise((resolve, reject) => {
      const args = [this.scriptPath, '--action', 'templates'];
      ...
      pythonProcess.on('close', (code) => {
        ...
        try {
          ...
          reject(new Error(`Failed to parse templates: ${(error as Error).message}`));
        }
      });
    });
  }
  ...
  async extractMedications(text: string): Promise<HealthcareExtraction[]> {
    const result = await this.extractText({
      text,
      instructions: 'Extract medication information',
      extractionType: 'medication'
    });

    return this.convertToHealthcareExtractions(result.extractions, 'medication');
  }

  async extractDiagnoses(text: string): Promise<HealthcareExtraction[]> {
    const result = await this.extractText({
      text,
      instructions: 'Extract diagnostic information',
      extractionType: 'diagnosis'
    });

    return this.convertToHealthcareExtractions(result.extractions, 'diagnosis');
  }

  async extractLabResults(text: string): Promise<HealthcareExtraction[]> {
    const result = await this.extractText({
      text,
      instructions: 'Extract laboratory results',
      extractionType: 'lab_result'
    });

    return this.convertToHealthcareExtractions(result.extractions, 'lab_result');
  }

  private convertToHealthcareExtractions(extractions: any[], type: string): HealthcareExtraction[] {
    return extractions.map((ext, index) => ({
      id: `${type}_${index}`,
      type: type as any,
      text: ext.extraction_text || '',
      startOffset: ext.start_offset || 0,
      endOffset: ext.end_offset || 0,
